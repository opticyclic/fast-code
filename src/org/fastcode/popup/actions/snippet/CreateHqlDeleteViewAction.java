...
public class CreateHqlDeleteViewAction extends CreateHqlGenericViewAction {

	/**
	 *
	 */
	public CreateHqlDeleteViewAction() {
		this.templateType = TEMPLATE_TYPE_CREATE_HQL_DELETE;
	...
	protected String[] getFieldTypesForHql() {
		return new String[] { "where_fields" };
	}

	/* (non-Javadoc)
	 * @see org.fastcode.popup.actions.snippet.CreateHqlGenericViewAction#getCreateSnippetData(java.lang.String)
	 */
	@Override
	public CreateSnippetData getCreateSnippetData(final String fileName) throws Exception {
		...
		if (validateTemplateSetting(templateSettings, fileName)) {
			...
			/*if (checkForErrors(createSnippetData.getToClass().getResource()) &&
					MessageDialog.openQuestion(shell, "Error", "There seems to be some problems associated with "  + createSnippetData.getToClass().getElementName()
																	+ ". It is better to fix those problems and try again. Want to abort?")) {
						return null;
			}*/
