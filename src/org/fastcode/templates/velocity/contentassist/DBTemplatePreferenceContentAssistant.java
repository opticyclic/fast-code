...
public class DBTemplatePreferenceContentAssistant extends AbstractTemplateContentAssistant {
	 ...
	 */
	public DBTemplatePreferenceContentAssistant(final Map<String, String> properties, final boolean propertiesOnly) {
	...
	public List<ICompletionProposal> getCompletionProposals(final IDocument document, final int offset, final int length, final String spaceToPad) {
		try {
			...
			if (element != null) {
				return DBTemplateReferenceManager.getCompletionProposals(element, offset, length, this.properties, this.propertiesOnly);
			}
